...
def sj():# function to create progress bars for the processes to display execution  and the run time information
    ...
    prog1 = Progressbar(parent,style="red.Horizontal.TProgressbar",orient = HORIZONTAL,length = 130, mode = 'determinate',maximum=output[0])
    ...
    prog2 = Progressbar(parent,style="blue.Horizontal.TProgressbar", orient = HORIZONTAL, length = 130, mode = 'determinate',maximum=output[1])
    prog2.place(x=334,y=500)
    pt2= str(output[1])
    progtime2 = Label(parent,text=pt2 +" seconds").place(x=384,y=510)
    s.configure("green.Horizontal.TProgressbar", foreground='green', background='green')
    prog3 = Progressbar(parent,style="green.Horizontal.TProgressbar", orient = HORIZONTAL, length = 130, mode = 'determinate',maximum=output[2])
    prog3.place(x=470,y=500)
    pt3= str(output[2])
    progtime3 = Label(parent,text=pt3 +" seconds").place(x=520,y=510)
    s.configure("black.Horizontal.TProgressbar", foreground='black', background='black')
    prog4 = Progressbar(parent,style="black.Horizontal.TProgressbar",orient = HORIZONTAL, length = 130, mode = 'determinate',maximum=output[3])
...
q = Entry(parent)
q.place(x=500,y=540)
qL = Label(parent,text="Quantum")
qL.place(x=550,y=560)
...
#Initialising intial array elements
output = []
priorityS=[]
...
def RRWT():
    ...
    for i in range(n):  
        rem_output[i] = output[i] 
    t = 0 # Current time  
  
    # traverse processes until done
    ...
    while(1): 
        ...
        for i in range(n): 
            ...
            if (rem_output[i] > 0): # If burst time of is greater than 0 then continue
                done = False # There is a pending process 
                  
                if (rem_output[i] > quantum) : 
                  
                   
                    t += quantum  #how many times a process has been processed
  
                    rem_output[i] -= quantum  #minus quantum
...
Button(parent,text="Load",command=lambda:[store(),proc(),RRAT(),RRTT,RRWT]).place(x = 640,y = 100)#Load values for Priority Calculations
